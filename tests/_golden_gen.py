"""Generate tests/recompute_golden.json — a frozen snapshot of recompute_schedule
outputs across fixed scenarios on all three networks. Run with the CURRENT engine
BEFORE optimizing; the gate test then asserts the optimized engine reproduces it
byte-for-byte. Regenerate only with explicit intent.

    python tests/_golden_gen.py
"""
...
OUT = os.path.join(HERE, "recompute_golden.json")
...
def scenarios():
    ...
    # an added-train case: exercises the empty-anomaly recompute + placement
...
def main():
    ...
    for label, net, trains, anomalies in scenarios():
        golden[label] = _serialize(recompute_schedule(net, trains, anomalies))
    with open(OUT, "w", encoding="utf-8") as f:
        json.dump(golden, f, indent=1, sort_keys=True)
    print(f"wrote {OUT} ({len(golden)} scenarios)")
